...
/*let filterLav = exercises.filter(el => el.impact.includes(myFilter))
    let filterMedium = exercises.filter(el => el.impact.includes(myFilter))
    let filterHigh = exercises.filter(el => el.impact.includes(myFilter))
...
//showAllExercises.addEventListener("click", showExercises)
